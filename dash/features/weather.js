...
class WeatherServer extends ServerFeature {
        ...
        tools.forEach(config, (value, key, data) => {
            switch (key) {
                ...
                case "daily":
                    ...
                    break;
                default:
                    this.log(`Unexpected configuration value! { ${key}: ${value}`);
            }
        });
        return true;
    }
    ...
    _setupClientFeature() {
        ...
        return parseIconMap().then((map) => {
            ...
            this.log(`Encountered error while parsing Weather Icon Map: ${err}`);
            return false;
        });
    }
    ...
    set updateInterval(interval) {
        if (interval > MINIMUM_UPDATE_INTERVAL) {
            this.log(`WARNING :: The new update interval value was lower than the minimum! Defaulting to ${MINIMUM_UPDATE_INTERVAL}.`);
